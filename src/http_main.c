...
void http_main_cnxn_cb(EV_P, void *cb_data, DB_conn *conn) {
	...
	if (strncmp(str_uri, "/env/upload", 12) == 0) {
		SFINISH_CHECK(
			(client->bol_public ? str_global_public_api_referer_list : str_global_api_referer_list)[0] == '*'
				|| client->str_referer != NULL
			, "Referer header required for this request"
		);
		SFINISH_CHECK(
			check_referer(client->str_referer, client->int_referer_len, (client->bol_public ? str_global_public_api_referer_list : str_global_api_referer_list))
		...
		SFINISH_CHECK(
			(client->bol_public ? str_global_public_api_referer_list : str_global_api_referer_list)[0] == '*' || client->str_referer != NULL
			, "Referer header required for this request"
		);
		SFINISH_CHECK(
			check_referer(client->str_referer, client->int_referer_len, (client->bol_public ? str_global_public_api_referer_list : str_global_api_referer_list))
		...
		SFINISH_CHECK((client->bol_public ? str_global_public_api_referer_list : str_global_api_referer_list)[0] == '*'
			|| client->str_referer != NULL
			, "Referer header required for this request"
		);
		SFINISH_CHECK(
		...
		if (
			...
			SFINISH_CHECK(
				...
				|| check_referer(
					...
					, (client->bol_public ? str_global_public_api_referer_list : str_global_api_referer_list)
				)
				, "Invalid Referer header"
			);
	...
	if (!bol_handoff && bol_error_state) {
        ...
        if (strstr(str_response, "You need to login") != NULL) {
			...
			SFINISH_SNCAT(
				...
				, "FATAL\015\012Session expired", strlen("FATAL\015\012Session expired")
			);
        }
        SFINISH_CHECK(
			build_http_response(
...
void http_main(EV_P, struct sock_ev_client *client) {
	...
	if (strncmp(str_uri, "/env/auth", 10) == 0 || strncmp(str_uri, "/env/auth/", 11) == 0 ||
		...
		SFINISH_CHECK(
			(client->conn = set_cnxn(EV_A, client, http_main_cnxn_cb)) != NULL
			, "set_cnxn failed"
		);
	} else {
		http_file_step1(EV_A, client);
...
bool http_client_info_cb(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(
		(str_json_version = jsonify(str_version, &int_json_version_len))
		, "jsonify failed"
	);

	int_json_user_len = int_user_len;
	SFINISH_CHECK(
		(str_json_user = jsonify(str_user, &int_json_user_len))
		, "jsonify failed"
	);

	int_json_port_len = strlen(str_global_port);
	SFINISH_CHECK(
		(str_json_port = jsonify(str_global_port, &int_json_port_len))
		, "jsonify failed"
	);
