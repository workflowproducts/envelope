	...
	if (client_request->bol_cancel_return == true) {
		...
		if (bol_success) {
			...
			if (client_request->parent->conn->copy_check != NULL && close_client_if_needed(client_request->parent, (ev_watcher *)&client_request->parent->conn->copy_check->check, EV_CHECK)) {
				...
				ev_idle_stop_debug(EV_A, &client_request->parent->conn->idle);
...
bool http_copy_check_cb(EV_P, bool bol_success, bool bol_last, void *cb_data, char *arg_str_response, size_t int_response_len) {
	...
	if (bol_success) {
		...
		if (bol_last) {
			...
			if ((int_response_write_len = write(client->int_sock, _str_response, client_request->int_current_response_length + int_header_len)) < 0) {
...
finish:
	...
	if (bol_error_state == true) {
		...
		SFREE(str_response);
		if ((int_response_write_len = write(client->int_sock, _str_response, strlen(_str_response) + int_header_len)) < 0) {
			SERROR_NORESPONSE("write() failed");
		}
		SFREE(_str_response);
