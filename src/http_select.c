...
void http_select_step1(EV_P, struct sock_ev_client *client) {
	...
	client->cur_request
		= create_request(
			client, NULL, NULL, NULL, NULL, sizeof(struct sock_ev_client_select)
			, ENVELOPE_REQ_SELECT, ws_select_free
		);
	...
	SFINISH_CHECK(
		DB_get_column_types_for_query(EV_A, client->conn, client_select->str_sql, client, http_select_step2)
	...
	if (DB_connection_driver(client->conn) != DB_DRIVER_POSTGRES) {
		...
		for (int_i = 1, int_len = DArray_end(client_select->darr_column_names); int_i < int_len; int_i += 1) {
			str_col = DB_escape_identifier(
				client->conn
				, DArray_get(client_select->darr_column_names, int_i)
				, strlen(DArray_get(client_select->darr_column_names, int_i))
			);
...
bool http_select_step3(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_SNCAT(
		client_copy_check->str_response, (size_t *)&client_copy_check->int_response_len
		, "{\"stat\": true, \"dat\": {\"arr_column\": ["
		, strlen("{\"stat\": true, \"dat\": {\"arr_column\": [")
		, str_json_columns, int_json_columns_len
		, "], \"dat\": [", (size_t)11
	);
...
finish:
	if (bol_error_state) {
        ...
        // this is used in the next step if we don't error
	    DB_free_result(res);
	}
    SFREE(str_response);
    // if client->str_http_header is non-empty, we are already taken care of
...
void http_select_step4(EV_P, ev_check *w, int revents) {
	...
	while (int_row < 10 && (status = DB_fetch_row(client_copy_check->res)) != DB_FETCH_END) {
		...
		if (client_select->str_row_count == NULL) {
			SFINISH_SNCAT(
				...
				, DArray_get(arr_row_values, 0)
				, strlen(DArray_get(arr_row_values, 0))
			);
		}
		...
		for (i = 1; i < int_len; i += 1) {
			...
			if (int_current_len == -1) {
				...
				strncmp(str_type, "smallint", 8) == 0 || strncmp(str_type, "integer", 7) == 0
	...
	if (status == DB_FETCH_END) {
        ...
        snprintf(str_content_len, 50, "%zu", (size_t)client_copy_check->int_response_len);
        ...
        client->str_http_response = client_copy_check->str_response;
        client->int_http_response_len = (size_t)client_copy_check->int_response_len;
        client_copy_check->str_response = NULL;
