...
void http_file_step1(struct sock_ev_client *client) {
	...
	if (client_http_file->int_file_fd == -1) {
		...
		ev_idle_start_debug(global_loop, &client_copy_check->idle);
	}
	bol_error_state = false;
finish:
#ifdef _WIN32
...
void http_file_write_cb(EV_P, ev_io *w, int revents) {
	...
	int_response_len = write(client->int_sock, client->str_response + client_http_file->int_written, (size_t)int_response_len);
