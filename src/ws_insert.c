...
void ws_insert_step1(EV_P, struct sock_ev_client_request *client_request) {
	...
	SFINISH_ERROR_CHECK(
		client_insert->str_real_table_name != NULL
		, "Query failed:\nFATAL\nerror_detail\tERROR: Failed to get table name from query.\n"
	);

	client_insert->str_table_alias
		= get_table_alias_name(
			client_request->ptr_query
	...
	SFINISH_ERROR_CHECK(
		client_insert->str_table_alias != NULL
		, "Query failed:\nFATAL\nerror_detail\tERROR: Failed to get table name from query.\n"
	);
	...
	*ptr_end_column_names = 0;
	...
	while (ptr_pk < ptr_end_pk) {
		...
		if (strncmp(str_col_seq, "''", 2) != 0) {
			if (DB_connection_driver(client_request->parent->conn) == DB_DRIVER_POSTGRES) {
				...
				SFINISH_SNCAT(
					str_temp_sequence, &int_temp_sequence_len
					, str_col_seq, int_col_seq_len
				);
	...
	if (DB_connection_driver(client_request->parent->conn) == DB_DRIVER_POSTGRES) {
		...
		SFINISH_CHECK(
			DB_exec(EV_A, client_request->parent->conn, client_request, str_sql, ws_insert_step2)
			, "DB_exec failed"
		);
	} else {
#ifndef ENVELOPE_INTERFACE_LIBPQ
...
bool ws_insert_step2(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(
		DB_copy_in(
			EV_A, client_request->parent->conn, client_request, client_insert->ptr_values
			, int_len_content, str_sql, ws_insert_step3
		)
		, "DB_copy_in failed"
	);
...
bool ws_insert_step3(EV_P, void *cb_data, DB_result *res) {
    ...
    if (DB_connection_driver(client_request->parent->conn) != DB_DRIVER_POSTGRES || DArray_count(client_insert->darr_sequence) == 0) {
...
bool ws_insert_view_step1(EV_P, struct sock_ev_client_request *client_request) {
	SDEBUG("ws_insert_view_step1: COPY IN FINISHED, CREATE SECOND TEMP TABLE");
	struct sock_ev_client_insert *client_insert = (struct sock_ev_client_insert *)(client_request->client_request_data);
	SDEFINE_VAR_ALL(str_sql);
	char *str_response = NULL;
	bool bol_ret = true;
	...
	// This will hold all of the new primary key values
	if (DB_connection_driver(client_request->parent->conn) == DB_DRIVER_POSTGRES) {
		SFINISH_SNCAT(
			str_sql, &int_sql_len
			, "CREATE TEMP TABLE ", (size_t)18
		...
		if (client_insert->str_identity_column_name != NULL) {
			...
			SFINISH_CHECK(
				DB_exec(
					EV_A, client_request->parent->conn, client_request, str_sql, ws_insert_view_step15_sql_server
				)
				, "DB_exec failed"
			);
		} else {
			...
			SFINISH_CHECK(query_is_safe(str_sql), "SQL Injection detected");
			SFINISH_CHECK(
				DB_exec(EV_A, client_request->parent->conn, client_request, str_sql, ws_insert_view_step2)
				, "DB_exec failed"
			);
		}
#endif
...
bool ws_insert_view_step2(EV_P, void *cb_data, DB_result *res) {
	SDEBUG("ws_insert_view_step2: SECOND TEMP TABLE CREATED, CREATING INSERT QUERIES");
	struct sock_ev_client_request *client_request = cb_data;
	struct sock_ev_client_insert *client_insert = (struct sock_ev_client_insert *)(client_request->client_request_data);
	char *str_response = NULL;
	bool bol_ret = true;
	...
	SFINISH_SNCAT(
		str_replace_table_name, &int_replace_table_name_len
		, client_insert->str_real_table_name, client_insert->int_real_table_name_len
		, ".", (size_t)1
	);
	SFINISH_SNCAT(
	...
	SFINISH_CHECK(
		DB_exec(
			EV_A, client_request->parent->conn, client_request
			, DArray_get(client_insert->darr_insert_queries, client_insert->int_current_insert_query++)
...
bool ws_insert_view_step4(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(
		DB_copy_out(
			EV_A, client_request->parent->conn, client_request, str_sql, ws_copy_check_cb
		)
		, "DB_copy_out failed!"
	);
...
bool ws_insert_table_step1(EV_P, struct sock_ev_client_request *client_request) {
	...
	SFINISH_CHECK(
		DB_exec(
			EV_A, client_request->parent->conn, client_request, str_sql, ws_insert_table_step2
		)
		, "DB_exec failed"
	);
