...
void http_insert_step1(struct sock_ev_client *client) {
	...
	if (str_data != NULL && client_insert->int_data_len > 0) {
		...
		while (ptr_loop[0] != 0) {
            ...
            if (strncmp(str_one_col, "id", 3) == 0) {
                SFREE(client_insert->str_sequence_name);
                client_insert->str_sequence_name = DB_escape_literal(client->conn, str_one_val, strlen(str_one_val));
                SFINISH_CHECK(client_insert->str_sequence_name != NULL, "DB_escape_literal failed");
                client_insert->bol_pk_provided = true;
            }
			SFREE(str_one_col);
			str_one_val = NULL;
		}
