...
void ws_delete_step1(EV_P, struct sock_ev_client_request *client_request) {
	...
	SFINISH_ERROR_CHECK(
		client_delete->str_real_table_name != NULL
	...
	if (DB_connection_driver(client_request->parent->conn) == DB_DRIVER_POSTGRES) {
		...
		SFINISH_CHECK(
			DB_exec(EV_A, client_request->parent->conn, client_request, str_sql, ws_delete_step2)
			, "DB_exec failed"
		);
	} else {
#ifndef ENVELOPE_INTERFACE_LIBPQ
...
// Start copying into temp table
	...
	SFINISH_CHECK(
		DB_copy_in(
			...
			, client_delete->ptr_query, int_len_content, str_sql, ws_delete_step3
		)
		, "DB_copy_in failed"
	);
...
bool ws_delete_step3(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(query_is_safe(str_sql), "SQL Injection detected");
	SFINISH_CHECK(
		DB_exec(EV_A, client_request->parent->conn, client_request, str_sql, ws_delete_step4)
		, "DB_exec failed"
	);
