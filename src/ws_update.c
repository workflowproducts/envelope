...
void ws_update_step1(EV_P, struct sock_ev_client_request *client_request) {
	...
	if (client_update->str_hash_columns != NULL) {
		...
		SFINISH_ERROR_CHECK(
			client_update->str_hash_where_clause != NULL
	...
	client_update->ptr_query = bstrstr(
		client_update->ptr_query
		, (size_t)(client_request->frame->int_length - (size_t)(client_update->ptr_query - client_request->frame->str_message))
		, "\012", (size_t)1
	);
		...
		*(ptr_pk_header - 1) = 0;
		...
		if (strncmp(str_pk_header, "pk", 2) == 0) {
			...
			if (client_update->str_identity_column_name != NULL && int_col_name_len == int_identity_column_name_len &&
				...
				if (DB_connection_driver(client_request->parent->conn) == DB_DRIVER_POSTGRES) {
					SFINISH_SNFCAT(
						client_update->str_pk_return_join_clause, &client_update->int_pk_return_join_clause_len
						, client_update->str_temp_table_name, client_update->int_temp_table_name_len
						, ".", (size_t)1
						, str_temp, strlen(str_temp)
						, " IS NOT DISTINCT FROM ", (size_t)22
					...
					SFINISH_SNFCAT(
						client_update->str_pk_return_where_clause, &client_update->int_pk_return_where_clause_len
						, client_update->str_real_table_name, client_update->int_real_table_name_len
						, ".", (size_t)1
						, str_col_name, int_col_name_len
						, " = ANY(array(SELECT ", (size_t)20
	...
	if (DB_connection_driver(client_request->parent->conn) == DB_DRIVER_POSTGRES) {
		...
		if (client_update->str_identity_column_name != NULL) {
			...
			SFINISH_CHECK(
				DB_exec(
					EV_A, client_request->parent->conn, client_request, str_sql, ws_update_step15_sql_server
				)
				, "DB_exec failed"
			);
		} else {
			...
			SFINISH_CHECK(
				DB_exec(
					EV_A, client_request->parent->conn, client_request, str_sql, ws_update_step2
				)
				, "DB_exec failed"
			);
		}
...
bool ws_update_step2(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(
		DB_copy_in(
			EV_A, client_request->parent->conn, client_request, client_update->ptr_query
			, int_len_content, str_sql, ws_update_step3
		)
		, "DB_copy_in failed"
	);
...
bool ws_update_step3(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(
		DB_exec(
			EV_A, client_request->parent->conn, client_request, str_sql, ws_update_step4
		)
		, "DB_exec failed"
	);
...
finish:
	...
	if (bol_error_state == true) {
		...
		WS_sendFrame(
			EV_A, client_request->parent, true, 0x01, str_response, strlen(str_response)
		);
		DArray_push(client_request->arr_response, str_response);
	}
	DB_free_result(res);
...
bool ws_update_step4(EV_P, void *cb_data, DB_result *res) {
	...
	SFINISH_CHECK(
		DB_exec(
			EV_A, client_request->parent->conn, client_request, str_sql, ws_update_step5
		)
		, "DB_exec failed"
	);
