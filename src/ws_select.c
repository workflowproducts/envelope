...
void ws_select_step1(EV_P, struct sock_ev_client_request *client_request) {
	...
	SFINISH_ERROR_CHECK(
		client_select->str_return_columns != NULL
	...
	SFINISH_ERROR_CHECK(
		client_select->str_return_escaped_columns != NULL
...
bool ws_select_step2(EV_P, void *cb_data, DB_result *res) {
	...
	if (DB_connection_driver(client_request->parent->conn) != DB_DRIVER_POSTGRES) {
		...
		SFINISH_SNFCAT(client_select->str_sql_escaped_return, &client_select->int_sql_escaped_return_len,
			"\012", (size_t)1);
		client_select->str_return_escaped_columns
		...
		if (client_select->str_order_by == NULL) {
			...
			for (size_t int_i = 0, int_len = DArray_end(arr_column_names); int_i < int_len; int_i += 1) {
				str_col = DB_escape_identifier(
					client_request->parent->conn, DArray_get(arr_column_names, int_i)
