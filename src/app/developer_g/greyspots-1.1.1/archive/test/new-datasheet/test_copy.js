...
// ############################################################################
    ...
    function updateHUD(element) {
        ...
        // update selection status control

        // convenience variable
        intOriginRecord = (
            element.internalSelection.originRecord
        );

        // we need the element that contains the selection status
        statusElement = findHudElement(element, 'text-selection-status');
    ...
    function databaseWSSELECTcolumnUnique(
        ...
        GS.ajaxJSON(
            ...
            function (data, error) {
                ...
                var strValue;
                var strType;
                var bolUncheckedFound;

                if (!error) {
                    // we need to convert the ajax JSON into TSV
                    data = data.dat;
                    arrData = [
                    ...
                    bolUncheckedFound = false;
                    i = 0;
                    len = data.dat.length;
                    while (i < len) {
                        strValue = data.dat[i][1];

                        //console.log('3***', jsnFilter);
                        //console.log('4***', jsnFilter.type);
                        //console.log('5***', jsnFilter.values);
                        ...
                        if (
                            ...
                            jsnFilter.values
                            //jsnFilter.values.length > 0
                            //  ^-- when nothing is selected, we still need to
                            //      test
                        ) {
                            index = jsnFilter.values.indexOf(strValue);
                            strType = jsnFilter.type;
                            ...
                            //console.log(
                            //    '6***',
                            //    jsnFilter.values.indexOf(strValue),
                            //    strValue,
                            //    strCheck,
                            //    jsnFilter.blanks
                            //);
        ...
        element.internalEvents.columnDropDown = function (event) {
            ...
            if (target.classList.contains('header-button')) {
                ...
                strHTML += (
                    ...
                    (
                        '<gs-button dialogclose remove-right remove-top ' +
                                    'inline>Cancel</gs-button>' +
                        '<gs-button dialogclose remove-left remove-top ' +
