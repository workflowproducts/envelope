...
// ############################################################################
    ...
    function renderScrollDimensions(element) {
        ...
        while (i < len) {
            ...
            if (arrColumnWidths[i] > intMaxColumnWidth) {
        ...
        while (i < len) {
            ...
            if (arrRecordHeights[i] > intMaxRecordHeight) {
...
// ############################################################################
    ...
    function databaseWSSELECTcolumnUnique(
        ...
        // I think that the current filter that we are choosing should not be in this select

        //strMessage = (
        //    'SELECT\t' +
        //        GS.encodeForTabDelimited(strSchema) + '\t' +
        //        GS.encodeForTabDelimited(strObject) +
        ...
        //    GS.encodeForTabDelimited(strWhere || '1=1') + '\t' +
        //    GS.encodeForTabDelimited(strColumn) + '\t' +
        //    GS.encodeForTabDelimited(strColumn + ' ASC') + '\t' +
        //    GS.encodeForTabDelimited(strLimit || 'ALL') + '\t' +
        //    GS.encodeForTabDelimited(strOffset || '0')
        //);
        ...
        GS.requestArbitrarySelectFromSocket(
            ...
            function (data, error) {
                if (!error) {
                    if (data.strMessage === 'TRANSACTION COMPLETED') {
                        // we want the select all to only be checked of all values
                        var arrData = tabData;
                        //console.log(bolUncheckedFound);
                        //      are checked
                        ...
                        //snapback - WAITING FOR ARBITUARY SQL IN WEBSOCKET
    
                        // we need to make the filter list container visible
                        uniqueFilterElement.removeAttribute('hidden');
    
    
                        valueListElement.addEventListener('initialized', function () {
                        ...
                        while (i < 15) {
                            ...
                            if (strRecord !== '' || strMessage !== '') {
                                if ((!jsnFilter || !jsnFilter.type)) {
                                    tabData.push('-1\t' + strRecord);
                                // nothing
                                } else if (jsnFilter.type === 'inclusion' && jsnFilter.values.length === 0) {
                                    if (jsnFilter.blanks && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('-1\t' + strRecord);
                                    } else if (jsnFilter.blanks === false && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('0\t' + strRecord);
                                ...
                                // everything
                                } else if (jsnFilter.type === 'exclusion' && jsnFilter.values.length === 0) {
                                    if (jsnFilter.blanks && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('-1\t' + strRecord);
                                    } else if (jsnFilter.blanks === false && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('0\t' + strRecord);
                                ...
                                } else if (jsnFilter && jsnFilter.type === 'inclusion') {
                                    if (jsnFilter.blanks && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('-1\t' + strRecord);
                                    } else if (jsnFilter.blanks === false && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('0\t' + strRecord);
                                        bolUncheckedFound = true;
                                    } else if (jsnFilter.values.indexOf(strRecord.split('\t')[1]) !== -1) {
                                        tabData.push('-1\t' + strRecord);
                                    } else {
                                        tabData.push('0\t' + strRecord);
                                        bolUncheckedFound = true;
                                    }
                                } else if (jsnFilter && jsnFilter.type === 'exclusion') {
                                    if (jsnFilter.blanks && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('-1\t' + strRecord);
                                    } else if (jsnFilter.blanks === false && strRecord.split('\t')[1] === '(blanks)') {
                                        tabData.push('0\t' + strRecord);
        ...
        element.internalEvents.columnContextMenu = function (event) {
            ...
            if (
                ...
                if (bolHideAndShow) {
                    ...
                    strHTML += (
                        '<div class="context-menu-header">Hide/Unhide Columns:</div>' +
                        '<div class="context-menu-indent">' +
                        '    <gs-button dialogclose remove-bottom' +
                            ...
                            if (strAnswer === 'Ok') {
                                ...
                                while (i < len) {
                                    ...
                                    if (strValue === '(blanks)') {
                                        ...
                                        //console.log(arrRecords[i], arrRecords[i][0]);
                                ...
                                //console.log(arrIncluded, arrExcluded);
                                //if element.internalData.columnListFilters[columnIndex] is undefined set to {} to prevent an error
                                if (!element.internalData.columnListFilters[columnIndex]) {
                                    element.internalData.columnListFilters[
                                        columnIndex
                                    ] = {};
                                }
                                ...
                                console.log(columnIndex, element.internalData);
...
// #############################################################################
        ...
        methods: {
            ...
            'clearFilter': function () {
                //TODO: michael is this correct?
                this.internalData.columnFilters = [];
                this.internalData.columnListFilters = {};
                // refresh the table
                dataSELECT(this);
            },
