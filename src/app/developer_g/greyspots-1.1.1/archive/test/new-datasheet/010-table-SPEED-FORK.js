...
//      author: Michael
//      epoch:  7/25/2017
//      reason: choppy scrolling performance
//      status: ABANDONED, INEFFECTIVE
//
// this is the SPEED FORK, what I am attemting with this fork is to reduce
//      time for style/layout recalculations by changing from absolutely
//      positioned cells to text flowed cells. this would require that all
//      cells be in the correct order, but, fixing order is a math issue
//      with extra node changes instead of a CSS issue with one node change.
//      another hope is that this will cause MS EDGE to stop acting like a
//      turtle epoxied to a floor and start acting like it's got the go-juice
//      to actually do something with it's life.
// As a side note, if you want to help the internet, stop using MS EDGE and
//      stop using IE. These browsers slow down desktop advancement.
// Here are the steps:
//      1) convert renderLocationFull to do the correct cell order
//      2) remove absolute CSS
//      3) remove CSS generation
//      4) after render, adjust the data viewport width/height to accommodate
//              the extra width/height of out-of-view cells
//      5) do performance check, if no performance improvement, stop, abandon
//              fork and leave the performance numbers below for future
//              reference.
//      6) convert renderLocationPartial to maintain cell order
//      7) do performance check, if no performance improvement, stop, abandon
//              fork and leave the performance numbers below for future
//              reference.
//
// Results:
//      this didn't cut down the time as far as easier methods. read more DOM
//          performance manuals.
//
// #############################################################################
// ############################### SPEED FORK #1 ###############################
...
//<br />
    ...
    function renderLocationFull(element) {
        ...
        // if there's a header: build column headings
        if (strHeaderTemplate) {
            ...
            strHTML += (strRecord + '<br />');
        }

        // if there is a record template: build cell elements
        if (strDataTemplate) {
        ...
        if (strInsertTemplate) {
            ...
            //      disabled: build top-left/select all cell
            if (!element.hasAttribute('no-record-selector')) {
                strHTML += (
    ...
    function renderLocationPartial(element) {
        ...
        /*
            scrolling up
                remove records from the top
                append records to the bottom
            scrolling down
                remove records from the bottom
                append records to the top
            scrolling left
    ...
    function renderLocation(element) {
        ...
        if (intViewportHeight < 3 || intViewportWidth < 3) {
            ...
            renderLocationFull(element);// testing
        }
        ...
        // in the future, we may allow individual record heights, not today
        if (arrRecordHeights[0]) {
            strCSS += (
                strCell + '[data-row-number] {' +
                'height:' + (
                    arrRecordHeights[0] + recordBorderHeight
                ) + 'px;' +
                '}'
            );
        }
