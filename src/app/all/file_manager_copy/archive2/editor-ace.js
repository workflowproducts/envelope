...
//jslint browser:true, white:false, this:true, maxlen:80
...
window.addEventListener('design-register-element', function () {
    ...
    window.designElementProperty_GSACE = function (selected) {
        ...
        var txt = function (attr) {
            return (
                '<gs-text class="target" value="' + encodeHTML(
                    selected.getAttribute(attr) || ''
                ) + '" mini></gs-text>'
            );
        };
        var chk = function (attr) {
            return (
                '<gs-checkbox class="target" ' +
                'value="' + (selected.hasAttribute(attr)) + '" ' +
                'mini></gs-checkbox>'
            );
        };
        var line = function (strTitle, strHTML, changeFunction, bolGlobal) {
...
// #############################################################################
    ...
    window.addEventListener('try-password', function (event) {
        ...
        var elems = xtag.query(document, 'gs-bace[encrypted="' + strKey + '"]');
        var i;
        var len;
